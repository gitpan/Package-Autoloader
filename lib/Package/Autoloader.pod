...
See the generator 'SQL_Table' for an attempt to have prototypes with AUTOLOAD.

Prototype checks occur at compile time, when AUTOLOAD isn't considered, yet. Therefore AUTOLOAD wasn't made aware of prototype definitions.
...
The statement 'use Package::Autoloader' is executed during compile time. Some things are not available then. Like the DATA file handle or lexical variables. You can use Package::Autoloader::again, see sql_table.pl and DATA_section.pl.
