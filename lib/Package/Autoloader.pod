...
By default the search path consists of the components of the package name. For the package A::B::C the name hierarchy is A::B::C, A::B::, A::, and ''. Any rule found on the way to the top is checked and if the check is successful, the search stops. You can modify the search path with the search proxy methods as described in L<Package::Autoloader::Path_Partition>.
...
The method register_rule has a convenience mode, where it accepts three-argument rule short cuts. The first argument is a generator, the second is a wildcard and the third is the matching subroutine pattern.
...
=item * a double colon followed by an asterisk b<'::*'> means the rule applies to package names deeper in the hierarchy, so that the rules defined in Synopis applies to Synopsis::Desc1;

=item * a combination of the above symbols B<'+::*'> means a combination of their functions; and

=item * a single asterisk B<'*'> means the rule applies globally other packages, including itself;
...
The statement 'use Package::Autoloader' is executed during compile time. Some things are not available then. Like the DATA file handle or lexical variables. Get the package object at run time with Package::Autoloader->new, see sql_table.pl and DATA_section.pl.
...
Uncomment the line with 'use MRO::Compat' and comment the one with 'use mro' to make Package::Autoloader work with older versions of Perl5.
