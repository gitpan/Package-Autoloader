...
Package::Autoloader::Generator::Constant_Functions - base class for generating constant functions at compile time
...
constant functions ATB_NAME, ..., ATB_PRICE.
	...
	use Package::Autoloader sub{eval shift}, sub {
	        my $generator = $_[0]->register_rule(
			'::Enumerated_CF', '=', 'ATB_');
	        $generator->configure(qw(NAME  SALE  STOCK  PRICE));
	        $_[0]->implement('ATB_', 'IS_');
	};
...
of your objects:
	...
	use Package::Autoloader sub{eval shift}, sub {
	        my $generator = $_[0]->register_rule(
			'::Random_CF', '=', 'ATB_');
	        $generator->configure(qw(NAME  TYPE  STOCK  PRICE));
	        $_[0]->implement('IS_');
	};
...
The following packages provide support for objects as array references and are therefore loosely related to Transporter: L<Class-Gomor> (there are more, but I can't find them right now).

The following packages treat arrays as objects, which Transporter is not about, but that also clarifies things: 
L<List::oo>, 
L<Class-Builtin>, 
L<Devel-Ladybug>, and
L<Object-Array>.
